...
This repo was started with the code from the Realtek USB driver `RTL8852AU_WiFi_linux_v1.15.0.1-0-g487ee886.20210714`. The current code improves on the Realtek code by reworking the debug output to avoid spamming the logs. In the current settings, messages from `RTW_ERR()`, `RTW_WARNING()`, and `RTW_WARNING()` will be output.
...
**NOTE:** The instructions below reference the old `lwfinger` repository. Please refer to the **active fork** at [https://github.com/natimerry/rtl8852au](https://github.com/natimerry/rtl8852au) for the most current and correct installation commands.
...
When a USB device is plugged in, or detected at boot, this rule causes the utulity `usb_modeswitch` to unload any **0bda:1a2b** devices that it finds. If you have a device with a different ID, change the rule accordingly.
...
2.  When prompted, enter the password you entered when creating the sign key.

If you enter the wrong password, your computer might not be bootable. In this case, use the **BOOT menu** from your BIOS, to boot into your OS then do below steps:
